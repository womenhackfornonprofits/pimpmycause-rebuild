!function () {
	...
	mainModule.controller("ui.sortable.sortableController", ["$scope", function ($scope) {
		this.scope = $scope, $scope.modelValue = null, $scope.callbacks = null, $scope.type = "sortable", $scope.options = {}, $scope.insertItem = function (index, itemData) {
		...
		return {
			...
			link: function (scope, element, attrs, ngModelController) {
				...
				}, !0), scope.$watch(attrs.isEnabled, function (newVal) {
					scope.isEnabled = void 0 !== newVal ? newVal : !0
				}, !0))
			}
		}
	})
		...
		return {
			...
			link: function (scope, element, attrs, itemController) {
				...
				function rollbackDragChanges() {
					placeElement.replaceWith(scope.itemScope.element), placeHolder.remove(), dragElement.remove(), dragElement = null, dragHandled = !1, containment.css("cursor", "")
				}

				var dragElement, placeHolder, placeElement, itemPosition, dragItemInfo, containment, dragListen, scrollableContainer, dragStart, dragMove, dragEnd, dragCancel, isDraggable, isDragBefore, isPlaceHolderPresent, bindDrag, unbindDrag, bindEvents, unBindEvents, hasTouch, dragHandled, isEnabled = !0;
				hasTouch = $window.hasOwnProperty("ontouchstart"), sortableConfig.handleClass && element.addClass(sortableConfig.handleClass), scope.itemScope = itemController.scope, scope.$watch("sortableScope.isEnabled", function (newVal) {
					isEnabled !== newVal && (isEnabled = newVal, isEnabled ? bindDrag() : unbindDrag())
					...
					if ((!hasTouch || !$helper.isTouchInvalid(event)) && dragHandled && dragElement) {
						if (event.preventDefault(), eventObj = $helper.eventObj(event), $helper.movePosition(eventObj, dragElement, itemPosition, containment, scrollableContainer), targetX = eventObj.pageX - $document[0].documentElement.scrollLeft, targetY = eventObj.pageY - ($window.pageYOffset || $document[0].documentElement.scrollTop), dragElement.addClass(sortableConfig.hiddenClass), $document[0].elementFromPoint(targetX, targetY), targetElement = angular.element($document[0].elementFromPoint(targetX, targetY)), dragElement.removeClass(sortableConfig.hiddenClass), targetScope = targetElement.scope(), !targetScope || !targetScope.type)return;
	...
	mainModule.controller("ui.sortable.sortableItemController", ["$scope", function ($scope) {
		...
		return {
			...
			link: function (scope, element, attrs, sortableController) {
				sortableConfig.itemClass && element.addClass(sortableConfig.itemClass), scope.sortableScope = sortableController.scope, scope.modelValue = sortableController.scope.modelValue[scope.$index], scope.element = element
			}
		}
	}])
}();