(function (e) {
		...
		return this.each(function () {
			...
			if (!e(this).hasClass("xdsoft_scroller_box")) {
				var r = function (e) {
					...
					return t
				}, i = 0, s = n.children().eq(0), o = n[0].clientHeight, u = s[0].offsetHeight, a = e('<div class="xdsoft_scrollbar"></div>'), f = e('<div class="xdsoft_scroller"></div>'), l = 100, c = false;
				a.append(f);
				...
				f.on("mousedown.xdsoft_scroller", function (r) {
					...
					var s = r.pageY, u = parseInt(f.css("margin-top")), c = a[0].offsetHeight;
				...
				n.on("touchmove", function (e) {
					if (c) {
						var t = r(e), i = Math.abs(parseInt(s.css("marginTop")));
	...
	e.fn.datetimepicker = function (n) {
		var r = 48, i = 57, s = 96, o = 105, u = 17, a = 46, f = 13, l = 27, c = 8, h = 37, p = 38, d = 39, v = 40, m = 9, g = 116, y = 65, b = 67, w = 86, E = 90, S = 89, x = false, T = e.isPlainObject(n) || !n ? e.extend(true, {}, t, n) : e.extend({}, t), N = 0, C = function (e) {
			...
			k.find(".xdsoft_month,.xdsoft_year").on("mousedown.xdsoft", function (t) {
				...
				var n = e(this).find(".xdsoft_select").eq(0), r = 0, i = 0;
			...
			n.setOptions = function (N) {
				...
				if (T.datepicker)C.addClass("active"); else C.removeClass("active");
				if (T.timepicker)A.addClass("active"); else A.removeClass("active");
				if (T.value) {
					t && t.val && t.val(T.value);
					j.setCurrentTime(T.value)
				}
				if (isNaN(T.dayOfWeekStart) || parseInt(T.dayOfWeekStart) < 0 || parseInt(T.dayOfWeekStart) > 6)T.dayOfWeekStart = 0; else T.dayOfWeekStart = parseInt(T.dayOfWeekStart);
					...
					switch (true) {
						...
						case e.type(T.mask) == "string":
							...
							t.on("keydown.xdsoft", function (n) {
								var N = this.value, C = n.which;
								switch (true) {
									case C >= r && C <= i || C >= s && C <= o || C == c || C == a:
										var k = O(this), L = C != c && C != a ? String.fromCharCode(s <= C && C <= o ? C - r : C) : "_";
										if ((C == c || C == a) && k) {
											k--;
											L = "_"
										}
										...
										if (D(T.mask, N)) {
											this.value = N;
											M(this, k)
										} else if (e.trim(N) == "")this.value = T.mask.replace(/[0-9]/g, "_"); else {
			...
			var j = new function () {
				...
				e.now = function () {
					var e = new Date;
					if (T.yearOffset)e.setFullYear(e.getFullYear() + T.yearOffset);
					return e
				};
				...
				e.strtodate = function (t) {
					if (t && t instanceof Date && e.isValidDate(t))return t;
					var n = t ? Date.parseDate(t, T.formatDate) : e.now();
					if (!e.isValidDate(n))n = e.now();
					return n
				};
			...
			A.find(".xdsoft_prev,.xdsoft_next").on("mousedown.xdsoft", function () {
				var t = e(this), n = 0, r = false, i = 110;
				(function s(e) {
					var o = O[0].clientHeight, u = M[0].offsetHeight, a = Math.abs(parseInt(M.css("marginTop")));
			...
			n.on("xchange.xdsoft", function (t) {
				...
				F = setTimeout(function () {
					var t = "", r = new Date(j.currentTime.getFullYear(), j.currentTime.getMonth(), 1, 12, 0, 0), i = 0, s = j.now();
					...
					while (i < j.currentTime.countDaysInMonth() || r.getDay() != T.dayOfWeekStart || j.currentTime.getMonth() == r.getMonth()) {
						...
						c = r.getMonth();
						h.push("xdsoft_date");
						if (u !== false && r > u || a !== false && r < a) {
							h.push("xdsoft_disabled")
						}
						if (j.currentTime.getMonth() != c) {
							h.push("xdsoft_other_month")
						}
					...
					var p = "", d = "", c = "", v = function (t, r) {
						...
						p += '<div class="xdsoft_time ' + h.join(" ") + '" data-hour="' + t + '" data-minute="' + r + '">' + i.dateFormat(T.formatTime) + "</div>"
					};
					if (!T.allowTimes || !e.isArray(T.allowTimes) || !T.allowTimes.length) {
						...
						for (var i = 0; i < T.allowTimes.length; i++) {
							d = j.strtotime(T.allowTimes[i]).getHours();
					...
					for (i = parseInt(T.yearStart, 10) + T.yearOffset; i <= parseInt(T.yearEnd, 10) + T.yearOffset; i++) {
						m += '<div class="xdsoft_option ' + (j.currentTime.getFullYear() == i ? "xdsoft_current" : "") + '" data-value="' + i + '">' + i + "</div>"
					}
					H.children().eq(0).html(m);
					for (i = 0, m = ""; i <= 11; i++) {
						m += '<div class="xdsoft_option ' + (j.currentTime.getMonth() == i ? "xdsoft_current" : "") + '" data-value="' + i + '">' + T.i18n[T.lang].months[i] + "</div>"
					}
					P.children().eq(0).html(m);
				...
				if (T.timepicker) {
					...
					if (e) {
						var t = O[0].clientHeight, n = M[0].offsetHeight, r = M.find(e).index() * T.timeHeightInTimePicker + 1;
						if (n - t < r)r = n - t;
			...
			n.mousewheel && C.mousewheel(function (e, t, n, r) {
				...
				if (t < 0)j.nextMonth(); else j.prevMonth();
				return false
			});
			n.mousewheel && O.unmousewheel().mousewheel(function (e, t, n, r) {
				if (!T.scrollTime)return true;
				var i = O[0].clientHeight, s = M[0].offsetHeight, o = Math.abs(parseInt(M.css("marginTop"))), u = true;
			...
			var U = function () {
				var t = n.data("input").offset(), r = t.top + n.data("input")[0].offsetHeight - 1, i = t.left, s = "absolute";
				if (T.fixed) {
