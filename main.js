...
/**
 * The main driver file for Pimp My Cause.  Provides the function necessary to
...
function Main(config){
    ...
    this.initDBConnections = function(cb){
        ...
        pb.dbm.getDb(pb.config.db.name, function(err, result){
            ...
            pb.log.debug('Main: Established connection to DB: %s', result.databaseName);
            cb(null, true);
        });
    };
    ...
    this.initDBIndices = function(cb) {
        if (pb.config.db.skip_index_check || !util.isArray(pb.config.db.indices)) {
            pb.log.info('Main: Skipping ensurance of indices');
            return cb(null, true);
        }

        pb.log.info('Main: Ensuring indices...');
        ...
        try{
            if (pb.config.server.ssl.enabled) {
                ...
                pb.handOffServer.listen(pb.config.server.ssl.handoff_port, function() {
                    pb.log.info('Main: Handoff HTTP server running on port: %d', pb.config.server.ssl.handoff_port);
                });
            }
            else {
            ...
            pb.server.listen(pb.config.sitePort, pb.config.siteIP, function() {
                pb.log.info('Main: %s running at site root [%s] on port [%d]', pb.config.siteName, pb.config.siteRoot, pb.config.sitePort);
    ...
    /**
     * Starts up the instance of Main
     * @method start
     */
    this.start = function() {
        var self = this;
