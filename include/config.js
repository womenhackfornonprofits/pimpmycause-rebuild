 ...
 * @type {String}
 */
Configuration.EXTERNAL_ROOT = path.join(path.sep, 'etc', 'pimpmycause');
 ...
 * @type {String}
 */
var LOG_FILE = path.join(LOG_DIR, 'pimpmycause.log');
...
var BASE_CONFIG = {
	...
	cluster: {
		...
		//The self managed flag indicates whether or not Main should
		...
		//Main to watch for failures and decide on its own whether or not
	...
	server: {
		...
		//response with "Main".  Cheesy but it helps the BuiltWith tools
	...
	//Main provides two storage engines out of the box.  The first is
	...
	//support the distributed Main configuration although it is not
